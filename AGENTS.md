...
The generated files are **gitignored — never edit them by hand**. Add a key to `intl_en.arb` only, with its `"@key": {"placeholders": ...}` metadata, and run `just gen_l10n`; a missing translation falls back to English and is not a CI failure. `just check_l10n` fails only on an ARB gen-l10n rejects or an empty `""` value.
