...
3. **`just check_l10n`** is what CI runs. It fails only on what the English fallback cannot absorb: an ARB that `flutter gen-l10n` rejects (malformed JSON, a locale code it cannot parse, a region file without its base language, unparseable ICU syntax) or an empty `""` translation, which would render as a blank label. Untranslated keys are listed per locale in the CI job summary as information, never as a failure.
